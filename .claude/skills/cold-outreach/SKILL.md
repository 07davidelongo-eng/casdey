...
**2026-08-12 run: correctly stopped, nothing sent.** The Routine was created before the autonomous-sending/reply-detection/service-account edits to this file were pushed, so its first run pulled the old skill (review checkpoint still required, inbox reading still forbidden) while its prompt asked for the opposite. It caught the contradiction and halted rather than guessing which instruction won, no email sent, no inbox read. Separately, that run also couldn't find `.env` or `gws` (correct, neither exists in a Routine environment) and didn't know to read credentials straight from process env instead, fixed in "Which environment am I running in?" above. Next scheduled run should work end to end; check its summary.
