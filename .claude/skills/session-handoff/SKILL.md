...
2. **Pull state from these sources (in order):**
   - Plan files referenced this session (check `C:\Users\GIUSEPPE\.claude\plans\` if a plan was mentioned).
   ...
   - Memory files written or updated (`C:\Users\GIUSEPPE\.claude\projects\<project>\memory\`).
