...
Covered in this pass: Overview, Settings (Data and privacy), the Offer page,
Campaigns including approving and sending one, Import, Members, the new
Calendar page. Two screenshots came with it: the Overview with the three trend
cards, and Data and privacy with the truncated search box. Verbatim below.

> #51: the issu ered box still appears
>
> #52: the search box with what happened, who and when thing is like cut midway... 2nd screen... the "when" cannot be seen on its entirety
>
> #53: you didn't expand the description under Give them a reason to come back as I asked you
>
> #54: one quick thing for the offers: I saw that you gave the ability to create different ones... but it isn't really working... just the box to create them... like there should be a 3rd button straight away that says like create another suggested offer (still leave that little one that gets shown at the bottom once you click "back to suggestions" anyway)
>
> #55: So for the campaign thing, okay, now it works like it is sending now the messages. One thing worth adding is that, um, when you, like, open a saved campaign, like, I click open and approve, I think that you should still put the button in the end, like, next to edit the message and delete this draft. You should, like, um, put some other buttons that are, like, safe changes and something like to save it for now, and you can get back to it later. Because, like, when I get in this page, like, I can either edit the message or just delete this draft. Like, what if I wanted to just, you know, edit the message, so edit the campaign and... okay. So I can see that I can save changes. What if now I want to just... instead of approving a start sending and just want to save it for later? k? Like, this is just the... this small thing here. Okay? And, also, uh, when I approved... oh, okay. So the button to get back to the campaigns is in the very top. I saw it just now. So wait. Okay. Yeah. Okay. So so you can get back to the campaigns. Anyway, just do that save for later stuff. Okay? And I also saw that when you create a draft and you click save as draft, basically, what I wanted already happened, which is, you know, you saved it. But I think that like that the back to campaigns button should be a little bit more evident or try to work out based also on what I said because I'm using the speech to text feature, and you're basically reading all the process that I went through while clicking around in that page.
>
> #56: I'm still not getting what you're saying with the point forty seven. like, uh, you're saying that this feature already exists. So how do I activate it? Is is it with the one message or one inch, uh, box that I have to take, like, the right each message individually box? Is that the thing that, uh, enables cast a personalization? If so, you should make it a little bit more evident. Okay? Like, a client, a gym needs to understand that by taking that box instead of having a fixed template they will have a more personalized one taken care by Castay. And so if this is the thing, just, you know, let let me know. Okay? And, also, um, for the anthropic account credit, use my personal account, like, uh, the one I'm using right now. Like, we will transfer this as soon as possible, as soon as you can.
>
> #57: Oh, and another important thing is that I think that in the settings, you should have somewhere where you can decide the language of the whole software like. Because there can be customers from all over Europe, so you should have the the ability to switch the language to their own language. Okay? So just make some translation, and we're all set.
>
> #58: In the import page, I'm seeing that, like, I like the format that you chose. The only problem right now is that the links that you attached to each software, they bring me to a invite page. Like, the the pages, like, don't exist. So try to fix this.
>
> #59: When I click the Undo button on the past import table, um, the little box that opens, like the description, the one that starts with Caste removes the x amount of members this file added, like the text stretches like too much. Like, it it should make a paragraph instead of a one line description because, like, I have a bar in the bottom part that lets me go left and right, which is very, very, um, anesthetic. Un... like, very non anesthetic. So just make it a paragraph and keep the box clean.
>
> #60: So in the members page, same same thing I ask you for some other pages. Like, you should limit the list shown to, like, ten members and then have those arrows or those pages so that you can scroll to the other ones. Because this way, let's say that a gym has, like, one thousand members, they will have to scroll for a decade before, like, seeing them. And I think that you should also have a search engine even here for this list so that you can filter out per member, per email, per last visit, per away, per visits, and per status. Okay? Like, I mean, the status is kinda... you can kinda see it in the full buttons that you added on top. But, anyway, just put this very precise filter search engine like we did with the other lists.
>
> #61: So I am very, very disappointed with the stuff you did in the overview page. Like, I asked you to do that cool Shopify style thing with the graphs and things, but you just added three boxes, which are very... look pretty bad, and you even moved the revenue recovered to the bottom, which doesn't make sense because that is the main thing that someone should look at. So first of all, I think that the revenue recorded box should be, like, below the five boxes, the members at risk, gone quiet, contacted, and returned boxes. And, like, it it should belong to that part... to that section. And then below that, you can have the, like, graphs and analytics section. Like, you need to put some graphs and some information like Shopify does. Okay? Like, you should really get inspired by Shopify. in terms of the dashboard and the overview because like like you did right now, it's shaped. Okay? Not shaped. s h i t not shaped
>
