...
/**
 * The WhatsApp side of roadmap #4. Unlike email's TestSendForm, there is no
 * single obvious "my own address" to default to, so the practice types the
 * number they want to test with. What lands on that number is the real
 * approved template, and replying to it rides the exact same webhook -> AI
    ...
    return (
      <Card>
        ...
        <p className="mt-1 text-[0.9375rem] text-graphite">
          WhatsApp is not turned on for this practice, so there is nothing to
          test yet.
        </p>
      </Card>
    );
  ...
  return (
    <Card>
      ...
      <p className="mt-1 mb-4 text-[0.9375rem] text-graphite">
        ...
        conversation continues exactly as it would for a real patient,
        including casdey&apos;s assistant and the hand-off when you say you want
        to book.
      </p>
      ...
      {state.sentTo ? (
        <div className="mt-4">
          <Notice>
            Sent to {state.sentTo}. Reply to it the same way a patient would,
