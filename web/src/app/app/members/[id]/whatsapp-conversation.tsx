...
/**
 * The hand-off surface for the WhatsApp channel (Track E1). Read-only:
 * booking itself is still recorded with the existing "Mark as returned"
 * action right below this on the member page (see ./actions-ui.tsx). This
 * card only tells the gym a conversation happened and, when the assistant
 * flagged booking intent, makes that impossible to miss.
 */
    ...
    <Card>
      ...
      {status === "booking_requested" ? (
        <p className="notice notice-info mt-2 mb-4">
          This member told casdey&apos;s assistant they want to come in. Get in
          touch to arrange a time, then mark them as returned below.
        </p>
      ) : (
        <p className="mt-1 mb-4 text-[0.875rem] text-stone">
          {status === "opted_out"
            ? "This member asked not to be contacted over WhatsApp again."
            : status === "closed"
...
function StatusPill({ status }: { status: WhatsAppConversationStatus }) {
  if (status === "booking_requested")
    return <Pill tone="returned">Wants to book</Pill>;
