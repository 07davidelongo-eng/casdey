/**
 * A member list sorted by time since last visit. The long tail on the right is
 * the part casdey works.
 *
 * This is a diagram, not a measurement. The shape is illustrative and labelled
 * as such: casdey has no gym data to chart yet, and inventing some would
...
export function LapsedChart({ className = "" }: { className?: string }) {
  const firstLapsed = BARS.findIndex((b) => b.lapsed);
  const bracketStart = START_X + firstLapsed * (BAR_W + GAP);
  ...
  return (
    <svg
      viewBox="0 0 380 186"
      role="img"
      aria-label="A member list sorted by time since last visit. Most recent visits sit on the left, and a long tail of members last seen over a year ago sits on the right. That tail is what casdey works."
      ...
      {BARS.map((bar, i) => {
        ...
        return (
          <rect
            ...
            height={bar.h}
            rx="3"
            className={bar.lapsed ? "fill-teal" : "fill-ash"}
          />
        );
      })}
