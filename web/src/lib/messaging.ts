 ...
 * practice inbox never will be). Resend can do both, so when a key is present
 * it is used and the patient sees their own practice rather than casdey.
 *
...
export type EmailProvider = {
  id: "resend" | "zoho";
  /** Whether a patient's reply reaches the practice directly. */
...
const resendProvider: EmailProvider = {
  ...
  async send(email) {
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        // The practice's name on casdey's verified domain: the patient sees who
