...
| D1 | Davide's uninterrupted self-serve walkthrough | Davide | **done 2026-09-07** — walked over six batches, **70 findings, all closed**; full record in `D1_WALKTHROUGH.md`. It caught two things nothing else had: the send queue broken on an upsert conflict target that no longer matched its index (every approve and test send failing `42P10`), and migrations `0029`/`0031` creating tables with RLS and no grants, hiding two shipped features. Remaining gap: the surfaces Davide did not walk (a member's booking flow, sign up, password reset, `/waitlist`, `/privacy`, processing terms) — an audit pass over those is still open. Original framing: Plain English: sign up at casdey.com as if you were a gym owner who has never seen it, with no help and no shortcuts, and see whether you get all the way from signup to an approved campaign without being confused or stuck. Everything else on this board exists to make that walk boring |
